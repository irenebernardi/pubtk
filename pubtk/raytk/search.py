...
from pubtk.runtk import UNIXINET_Dispatcher
...
def ray_trial(config, label, dispatcher_constructor, project_path, output_path, submit):
    ...
    tid = (tid.split('_')[-1])  # integer value for the trial, TODO figure out if removal of int is relevant
    ...
    #dispatcher = dispatcher_constructor(project_path=project_path, output_path=output_path, submit=submit,
                                       # gid=run_label)
    def create_dispatcher(submit, project_path, socket_type='UNIX'):
        return UNIXINET_Dispatcher(submit=submit, project_path=project_path, socket_type=socket_type)

    dispatcher = create_dispatcher(submit, project_path, 'UNIX')
